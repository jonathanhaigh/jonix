...
*
* Note: function pci_dev_lookup_ids() is adapted from
* http://www.pcidatabase.com/reports.php?type=c-header
* written by
* William H. Avery III (aka Highlander) at Altitech
* --------------------------------------------------*/
...
/*  ----------------------------------------------------
 *  Function:       pci_dev_lookup_ids
 *
 *  Note: Adapted from
 *  http://www.pcidatabase.com/reports.php?type=c-header
 *  --------------------------------------------------*/
void pci_dev_lookup_ids(pci_dev_t *dev){

	int	i;

    dev->class_desc     = &null_str;
    dev->subclass_desc  = &null_str;
    dev->prog_if_desc   = &null_str;
    dev->vendor_short   = &null_str;
    dev->vendor_long    = &null_str;
    dev->name           = &null_str;
    dev->desc           = &null_str;
    ...
    // Lookup vendor
    //
	for(i=0; i<PCI_VENDOR_IDS_LEN; i++){

		if(pci_vendor_ids[i].vendor_id == dev->vendor_id){
            dev->vendor_short   = &(pci_vendor_ids[i].vendor_short);
            dev->vendor_long    = &(pci_vendor_ids[i].vendor_long);
		}
	}

    // Lookup device
    //
	for(i=0; i<PCI_DEV_IDS_LEN; i++){

		if(pci_dev_ids[i].dev_id == dev->dev_id && pci_dev_ids[i].vendor_id == dev->vendor_id){
            dev->name   = &(pci_dev_ids[i].name);
            dev->desc   = &(pci_dev_ids[i].desc);
		}
	}

    // Lookup class/subclass/prog_if
    //
	for(i=0; i<PCI_CLASS_IDS_LEN; i++){

		if(pci_class_ids[i].class_id == dev->class_id){
            
			if(!**(dev->class_desc)){
                dev->class_desc  = &(pci_class_ids[i].class_desc);
            }

			if(pci_class_ids[i].subclass_id == dev->subclass_id){

				if(!**(dev->subclass_desc)) dev->subclass_desc = &(pci_class_ids[i].subclass_desc);

				if(pci_class_ids[i].prog_if == dev->prog_if){
					dev->prog_if_desc = &(pci_class_ids[i].prog_if_desc);
					break;
				}
			}
		}
